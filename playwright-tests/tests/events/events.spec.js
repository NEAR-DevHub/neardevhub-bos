...
test.describe("Wallet is connected, but not KYC verified", () => {
  ...
  test("should be able to blur 'get verified' drop-down", async ({ page }) => {
    ...
    await page.goto("/events-commitee.near/widget/app?page=create-proposal");
...
test.describe("Don't ask again enabled", () => {
  test.use({
    storageState:
      "playwright-tests/storage-states/wallet-connected-with-events-access-key.json",
  });
  test("should create a proposal", async ({ page }) => {
    ...
    await page.goto("/events-commitee.near/widget/app?page=proposals");

    const widgetSrc =
      "events-commitee.near/widget/devhub.entity.proposal.Editor";

    await setDontAskAgainCacheValues({
      page,
      widgetSrc,
      methodName: "add_proposal",
      contractId: "events-commitee.near",
    });
    ...
    const transaction_toast = await page.getByText(
      "Calling contract events-commitee.near with method add_proposal"
    );
  ...
  test("should add comment on a proposal", async ({ page }) => {
    ...
    await page.goto("/events-commitee.near/widget/app?page=proposal&id=17");
    const widgetSrc =
      "events-commitee.near/widget/devhub.entity.proposal.ComposeComment";
...
test.describe('Moderator with "Don\'t ask again" enabled', () => {
  ...
  test("should edit proposal timeline", async ({ page }) => {
    ...
    await setDontAskAgainCacheValues({
      page,
      contractId: "events-commitee.near",
      widgetSrc: "events-commitee.near/widget/devhub.entity.proposal.Proposal",
...
test.describe("Wallet is connected", () => {
  test.use({
    storageState:
      "playwright-tests/storage-states/wallet-connected-events.json",
  });
      ...
      JSON.stringify(
        {
          ...
          body: {
            ...
            name: "Test proposal 123456",
            description:
              "The test proposal description. And referencing [#2 DevHub Developer Contributor report by Thomas for 03/11/2024 – 04/12/2024](https://near.social/events-commitee.near/widget/app?page=proposal&id=2)",
            category: "DevDAO Platform",
            summary: "Test proposal summary 123456789",
            linked_proposals: [],
            requested_sponsorship_usd_amount: "12345",
