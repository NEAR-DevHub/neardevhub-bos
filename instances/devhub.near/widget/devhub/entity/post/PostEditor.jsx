...
useEffect(() => {
  if (mode == "Edit") {
    const data = Near.view("${alias_REPL_DEVHUB_LEGACY}", "get_post", {
      post_id: postId,
    });
    if (!postData) {
      setPostData(data);
...
const checkLabel = (label) => {
  Near.asyncView("${alias_REPL_DEVHUB_LEGACY}", "is_allowed_to_use_labels", {
...
const existingLabelStrings =
  Near.view("${alias_REPL_DEVHUB_LEGACY}", "get_all_allowed_labels", {
    editor: context.accountId,
  }) ?? [];
      ...
      allowNew={(results, props) => {
        return (
          ...
          props.selected.filter((selected) => selected.name === props.text)
            .length == 0 &&
          Near.view("${alias_REPL_DEVHUB_LEGACY}", "is_allowed_to_use_labels", {
            editor: context.accountId,
            labels: [props.text],
          })
        );
...
const callDescriptionDiv = () => {
  return (
    <div className="col-lg-12  mb-2">
      ...
      <Widget
        src={
          "${alias_REPL_DEVHUB}/widget/devhub.components.molecule.MarkdownEditor"
        }
        props={{
