  ...
  <Navbar className="position-relative">
    ...
    <div className="d-flex gap-3 align-items-center">
      <LinksContainer>
        {links.map((link) => (
          <Widget
            src="${alias_REPL_DEVHUB}/widget/devhub.components.molecule.NavbarDropdown"
            props={{
              title: link.title,
              href: link.href,
              links: link.links,
          ...
          {links.map((link, idx) =>
            link.href ? (
              <MobileLink
                ...
                href={`/${alias_REPL_DEVHUB}/widget/app?page=${link.href}`}
              >
                {link.title}
              </MobileLink>
            ) : (
              link.links.map((it, idx) =>
                ...
                it.href.startsWith("https://") ? (
                  ...
                  <MobileLink
                    key={`nested-link-${idx}`}
                    className={link.href === props.page && "active"}
                    href={`/${alias_REPL_DEVHUB}/widget/app?page=${it.href}`}
                  >
                    /{it.title}
                  </MobileLink>
                )
