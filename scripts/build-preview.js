...
if (!ACCOUNT_ID) {
    console.error("Error: Account is not provided. Please provide the account to deploy the widgets to.");
    process.exit(1);
}
...
if (NETWORK_ENV === "testnet" && !CONTRACT_ID) {
    console.error("Error: Network is set to testnet but no contract is provided. Please specify a contract to use with testnet.");
    process.exit(1);
}
...
if (fs.existsSync(REPLACEMENTS_JSON)) {
    ...
    fs.writeFileSync(`${REPLACEMENTS_JSON}.tmp`, JSON.stringify(replacements, null, 2));
} else {
...
let replacements = JSON.parse(fs.readFileSync(`${REPLACEMENTS_JSON}.tmp`, 'utf8'));
