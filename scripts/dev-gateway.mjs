...
const replaceRpc = async (htmlfile) => {
  ...
  indexHtmlData = indexHtmlData.replace('<near-social-viewer></near-social-viewer>', '<near-social-viewer rpc="http://localhost:20000"></near-social-viewer>');
...
await writeFile(replacementsWithEnvFileName, JSON.stringify(
  Object.assign(replacements, { 'REPL_POSTHOG_API_KEY': process.env['POSTHOG_API_KEY'], 'REPL_RPC_URL': 'http://localhost:20000' })
  , null, 1)
);
