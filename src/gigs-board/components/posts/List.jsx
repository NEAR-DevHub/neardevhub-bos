...
const allPosts = (Near.view(nearDevGovGigsContractAccountId, "get_all_post_ids") ?? []).reverse();
const allTopPosts = (Near.view(nearDevGovGigsContractAccountId, "get_children_ids") ?? []).reverse();

const postIds = props.label
  ? Near.view(nearDevGovGigsContractAccountId, "get_posts_by_label", { label: props.label }).reverse()
  : props.recency == "all"
  ? allPosts
  : allTopPosts;
...
return (
  <div>
    {postIds.map((postId) => widget(`components.posts.Post`, { id: postId, referral: props.referral, expandable: true, defaultExpanded: false }, postId))}
  </div>
);
